...
\subsection{Interface for Document Classes}\label{aut:clsint}
Some features of \biblatex\ require interaction with the document class in use. A few of these interactions may be subtle because they involve patching class commands, others are more visible, for example when \biblatex\ typesets the bibliography headings.
For a long time \biblatex\ has featured extensive support not only for the standard document classes, but also for \acr{KOMA}-Script and \sty{memoir}. Because it is not viable for \biblatex\ to have ready special code for all possible document classes, this section offers a different approach: A standardised interface for document classes.

Currently the interface only manages the appearance of the bibliography headings.
The advantage of this interface over using \cmd{defbibheading} directly is that \biblatex\ does not need to be loaded to use it. This means that classes do not have to defer these settings to \cmd{AtEndPreamble}---they can just work linearly, \biblatex\ will pick up the definitions when it is loaded. Naturally this implies that the definitions have to be made before \biblatex\ is loaded. For a general-purpose document class this seems like a good approach anyway, since it probably does not load any bibliography packages itself and leaves that decision to the end user.

The heading interface allows document classes to regain control over the bibliography headings.
Classes that want to make use of the heading interface define the following macros for each of the headings listed in the description of \cmd{defbibheading} in \secref{use:bib:hdg} they want to take over. These macros must be defined before \biblatex\ is loaded.
\begin{marglist}

\item[\cmd{abx@clsinterface@bibheading@\ensuremath\langle name\ensuremath\rangle}] This macro, which must ultimately take one (mandatory) argument, is used as the \prm{code} argument to \cmd{defbibheading}. The argument is the title text of the heading.
A class that wants the standard bibliography heading \opt{bibliography} typeset as a normal section could use
\begin{ltxexample}
\newcommand*{\abx@clsinterface@bibheading@bibliography}[1]{\section{#1}}
\end{ltxexample}

\item[\cmd{abx@clsinterface@bibheading@\ensuremath\langle name\ensuremath\rangle @title}] This macro is used as the optional \prm{title} argument to \cmd{defbibheading}. Usually this macro should simply expand to \cmd{refname}, \cmd{bibname} or \cmd{biblistname}. If this macro is not defined, \cmd{defbibliography} will be executed without the optional \prm{title} argument and defaults to \cmd{bibname}.
\begin{ltxexample}
\newcommand*{\abx@clsinterface@bibheading@bibliography@title}{\refname}
\end{ltxexample}
Would define the default title of \opt{bibliography} to be \cmd{refname}.

\item[\cmd{abx@clsinterface@defaultheading@bib}] The default heading used by \cmd{printbibliography} if the \prm{heading} option is not set. The value of this command is simply the \prm{name} of a bibliography heading.

\item[\cmd{abx@clsinterface@defaultheading@bibheading}] The default heading for \cmd{printbibheading} if the \prm{heading} option is not set.

\item[\cmd{abx@clsinterface@defaultheading@biblist}] The default heading used by \cmd{printbiblist} if the \prm{heading} option is not set.

\item[\cmd{abx@clsinterface@defaultheading@bibbysection}] The default heading used by \cmd{bibbysection} if the \prm{heading} option is not set.

\item[\cmd{abx@clsinterface@defaultheading@bibbysegment}] The default heading used by \cmd{bibbysegment} if the \prm{heading} option is not set.

\item[\cmd{abx@clsinterface@defaultheading@bibbycategory}] The default heading used by \cmd{bibbycategory} if the \prm{heading} option is not set.
\end{marglist}
...
\item Added an interface for document classes\see{aut:clsint}
