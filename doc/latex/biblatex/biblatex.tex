...
The length registers and counters in this section may be changed with \cmd{setlength} and \cmd{setcounter}, respectively.
Length and counter definitions with \cmd{setlength} and \cmd{setcounter}, respectively, perform a full expansion of the new value on assignment. In some cases it may not be desirable to use the values active in the preamble, instead the values from the document body should be picked up. \cmd{DeferSetBibLength} allows for a way to defer the length assignment to the beginning of the bibliography environment.

\begin{ltxsyntax}

\cmditem{DeferSetBibLength}{length}{value}
\cmditem*{DeferSetBibLength}*{length}{value}

Defers the assignment of \prm{value} to \prm{length} to the beginning of the bibliography environment. The value of \prm{length} should \emph{not} be used outside of bibliography environments as it will be set to a dummy value that acts as a sentinel value. The starred version clears the list of all deferred assignments by performing all deferred assignments now before it adds a new one. The assignment of the lengths can be triggered manually with \cmd{SetDeferredBibLengths}.
%
\begin{ltxexample}
\DeferSetBibLength{\bibhang}
  {\ifnumequal{\parindent}{0}{1em}{\parindent}}
\end{ltxexample}
%

\cmditem{ClearDeferredSetBibLengths}
\cmditem*{ClearDeferredSetBibLengths}*

Clears the list of all deferred length assignments. To avoid leaving the cleared lengths with nonsensical sentinel values the unstarred version performs all deferred assignments before clearing the list. The starred version does not perform the assignments and may therefore leave the lengths in a practically useless state. In practice this command will probably not be used very often since a deferred assignment can already be blocked by setting a \prm{length} to a value different from the dummy value.

\cmditem{SetDeferredBibLengths}

This command performs deferred length assignments for all lengths which are currently set to the sentinel value.

This command is automatically executed at the beginning of each bibliography environment defined with \cmd{defbibenvironment}.

\lenitem{bibhang}

The hanging indentation of the bibliography, if applicable. This length is initialized with \cmd{DeferSetBibLength} at load-time. The default value is \cmd{parindent}. If \cmd{parindent} is zero, \cmd{bibhang} is set to \texttt{1em} instead.

\lenitem{biblabelsep}

The horizontal space between entries and their corresponding labels in the bibliography. This only applies to bibliography styles which print labels, such as the \texttt{numeric} and \texttt{alphabetic} styles. This length is initialized to twice the value of \cmd{labelsep} with \cmd{DeferSetBibLength} at load-time.

\lenitem{bibitemsep}

The vertical space between the individual entries in the bibliography. This length is initialized to \cmd{itemsep} with \cmd{DeferSetBibLength} at load-time. Note that \len{bibitemsep}, \len{bibnamesep}, and \len{bibinitsep} obey the rules for \cmd{addvspace}, that is, when vertical space introduced by any of these commands immediately follows on from space introduced by another of them, the resulting total space is equal to the largest of them.
...
\begin{changelog}
\begin{release}{?.??}{????-??-??}
\item Added deferred length assignments with \cmd{DeferSetBibLength}\see{use:fmt:len}
\end{release}
